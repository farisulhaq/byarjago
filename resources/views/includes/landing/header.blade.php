<section
    class="h-full w-full border-box transition-all duration-500 linear lg:px-16 md:px-20 px-8 py-8 bg-serv-bg">
    <div class="navbar-1-1" style="font-family: 'Poppins', sans-serif">
        <div class=" mx-auto flex flex-wrap flex-row items-center justify-between text-white">
            ...
            <div class="hidden lg:flex lg:items-center lg:w-auto w-full lg:ml-auto lg:mr-auto flex-wrap items-center text-base justify-center"
                ...
                <nav
                    ...
                    <a href="{{ route('index') }}" class="block {{ request()->is('/') ? 'nav-link active font-medium' : 'nav-link teByarJago-text' }}">Home</a>
                    <a href="{{ route('explore.landing') }}" class="block {{ request()->is('explore') ? 'nav-link active font-medium' : 'nav-link teByarJago-text' }}">Explore</a>

                    @auth
                        <hr class="block lg:hidden">
                        <a href="{{ route('member.dashboard.index') }}" class="block lg:hidden nav-link" >Dashboard</a>

                        <a href="{{ route('logout') }}" class="block lg:hidden nav-link" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Logout
            ...
            @guest
                <div class="hidden lg:flex lg:items-center lg:w-auto w-full" id="menu">
                    <button
                        ...
                        class="teByarJago-login-text items-center border-0 block lg:inline-block lg:py-3 lg:px-10 focus:outline-none rounded-2xl font-medium text-base mt-6 lg:mt-0">
                        Log In
                    </button>

                    <button
                        onclick="toggleModal('registerModal')"
                        class="lg:ByarJagoices-bg teByarJago-login-text items-center border-0 block lg:inline-block  lg:py-3 lg:px-10 focus:outline-none rounded-2xl font-medium text-base mt-6 lg:mt-0">
                        Sign Up
                    </button>
                </div>
            @endguest
